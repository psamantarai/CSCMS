...
def test_untouched_date_reads_as_open():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        status = get_day_status("2026-10-01", conn)
...
def test_close_with_no_variance_needs_no_remarks():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _fund_cash(conn, cash_id, 500000, "2026-10-05")

        result = close_day("2026-10-05", CloseDayRequest(physical_cash_paise=500000), conn)
        assert result["status"] == "closed"
        assert result["cash_variance_paise"] == 0

        status = get_day_status("2026-10-05", conn)
...
def test_close_with_variance_requires_remarks():
    with tempfile.TemporaryDirectory() as tmp:
        conn = _seeded_conn(Path(tmp))
        cash_id = conn.execute("SELECT id FROM accounts WHERE name = 'Cash Drawer'").fetchone()[0]
        _fund_cash(conn, cash_id, 500000, "2026-10-06")

        try:
            close_day("2026-10-06", CloseDayRequest(physical_cash_paise=499000), conn)
        ...
        status = get_day_status("2026-10-06", conn)
        assert status["status"] == "open"

        result = close_day("2026-10-06", CloseDayRequest(physical_cash_paise=499000, remarks="counted short"), conn)
        assert result["status"] == "closed"
...
def test_double_close_is_rejected():
    with tempfile.TemporaryDirectory() as tmp:
        conn = _seeded_conn(Path(tmp))
        cash_id = conn.execute("SELECT id FROM accounts WHERE name = 'Cash Drawer'").fetchone()[0]
        _fund_cash(conn, cash_id, 100000, "2026-10-07")
        close_day("2026-10-07", CloseDayRequest(physical_cash_paise=100000), conn)
...
def test_snapshot_matches_derived_balances_exactly():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        conn = _seeded_conn(Path(tmp))
        cash_id = conn.execute("SELECT id FROM accounts WHERE name = 'Cash Drawer'").fetchone()[0]
        sbi_id = _fund_cash(conn, cash_id, 800000, "2026-10-10")
        create_expense(ExpenseCreate(business_date="2026-10-10", category="Rent", amount_paise=50000, account_id=cash_id), conn)

        close_day("2026-10-10", CloseDayRequest(physical_cash_paise=750000), conn)

        for account_id in (cash_id, sbi_id):
            snapshot = conn.execute(
                "SELECT * FROM daily_account_balance WHERE business_date = ? AND account_id = ?",
                ("2026-10-10", account_id),
            ).fetchone()
            direct = conn.execute(
                "SELECT SUM(amount_paise) FROM ledger WHERE account_id = ? AND business_date <= ?",
                (account_id, "2026-10-10"),
            ).fetchone()[0] or 0
...
def test_report_reconciles_with_the_ledger_before_and_after_close():
    with tempfile.TemporaryDirectory() as tmp:
        conn = _seeded_conn(Path(tmp))
        cash_id = conn.execute("SELECT id FROM accounts WHERE name = 'Cash Drawer'").fetchone()[0]
        sbi_id = _fund_cash(conn, cash_id, 300000, "2026-10-12")
        create_expense(ExpenseCreate(business_date="2026-10-12", category="Ink", amount_paise=10000, account_id=cash_id), conn)

        # report on an OPEN day: still a live derivation, not blocked
        report = get_day_report("2026-10-12", conn)
        ...
        close_day("2026-10-12", CloseDayRequest(physical_cash_paise=290000), conn)
        report_after = get_day_report("2026-10-12", conn)
...
def test_opening_balance_for_the_next_day_equals_this_days_closing():
    with tempfile.TemporaryDirectory() as tmp:
        conn = _seeded_conn(Path(tmp))
        cash_id = conn.execute("SELECT id FROM accounts WHERE name = 'Cash Drawer'").fetchone()[0]
        _fund_cash(conn, cash_id, 400000, "2026-10-15")
        close_day("2026-10-15", CloseDayRequest(physical_cash_paise=400000), conn)

        from app.ledger import opening_balance
        assert opening_balance(conn, cash_id, "2026-10-16") == 400000
        conn.close()
