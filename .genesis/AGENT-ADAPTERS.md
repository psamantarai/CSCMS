...
**CSCMS override:** this repo vendors its own copy in-tree instead of relying on the global
locations above — `.genesis/vendor/{agentic-swe-kit,genesis-kit,skills-directory}/`. Read
`agentic-swe-master` from `.genesis/vendor/agentic-swe-kit/skills/orchestrator/agentic-swe-master/`
and cognitive skills from `.genesis/vendor/skills-directory/skills/`; neither is installed as a
project skill in `.claude/skills/`, so load them by pasting `SKILL.md`'s body into context per the
"any other agent" row above. To re-run `scaffold.sh`/`graphizer.mjs` later, they live in
`.genesis/vendor/genesis-kit/tools/`.
