...
# Humble Object Pattern: Split behaviors into two modules — one humble (hard-to-test, stripped to bare essence) and one testable (all the logic that was extracted from the humble object)
...
## PresenterView Split The

# Presenter/View Split: The View is the humble object that only moves pre-formatted data to the screen; the Presenter is the testable object that formats all data into a View Model of strings, booleans, and enums
...
# View Model: A simple data structure containing only strings, booleans, and enums, fully prepared by the Presenter so the View has zero logic to perform
