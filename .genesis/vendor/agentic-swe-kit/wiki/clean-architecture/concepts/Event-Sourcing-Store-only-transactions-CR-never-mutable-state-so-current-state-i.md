---
title: Event Sourcing: Store only transactions (CR), never mutable state, so current state is always derived by replaying transactions from a known checkpoint
created: 2026-05-12
updated: 2026-05-12
type: concept
...
# Event Sourcing: Store only transactions (CR), never mutable state, so current state is always derived by replaying transactions from a known checkpoint
