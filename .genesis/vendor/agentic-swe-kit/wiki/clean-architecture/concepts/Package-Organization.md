---
title: Package by Component: Hybrid approach bundling all responsibilities of a coarse-grained component into a single package, using access modifiers to let the compiler enforce architectural rules
created: 2026-05-12
updated: 2026-05-12
type: concept
...
# Package by Component: Hybrid approach bundling all responsibilities of a coarse-grained component into a single package, using access modifiers to let the compiler enforce architectural rules
...
# Package by Layer: Horizontal slicing of code into technical layers (web, service, persistence); simple to start but doesn't reflect business domain and enables architectural bypass
...
# Ports and Adapters (Hexagonal Architecture): Code organized into 'inside' (domain) and 'outside' (infrastructure) regions where outside always depends on inside, never the reverse
...
# Périphérique Anti-Pattern: Placing all infrastructure code in one source tree allows infrastructure components to call each other directly, bypassing the domain entirely
