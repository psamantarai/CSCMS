---
title: Key Diversification: Derive per-device keys by encrypting a device ID under a master vend key (KID = {ID}KV), so compromise of one device does not expose others
created: 2026-05-12
updated: 2026-05-12
type: concept
...
# Key Diversification: Derive per-device keys by encrypting a device ID under a master vend key (KID = {ID}KV), so compromise of one device does not expose others
