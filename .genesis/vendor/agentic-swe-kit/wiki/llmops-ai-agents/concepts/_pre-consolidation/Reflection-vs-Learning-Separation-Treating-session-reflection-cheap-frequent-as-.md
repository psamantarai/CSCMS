---
title: Reflection vs. Learning Separation: Treating session reflection (cheap, frequent) as distinct from code/prompt changes (expensive, gated) so that observation never directly mutates agent behavior without an intervening approval step
created: 2026-05-12
updated: 2026-05-12
type: concept
...
# Reflection vs. Learning Separation: Treating session reflection (cheap, frequent) as distinct from code/prompt changes (expensive, gated) so that observation never directly mutates agent behavior without an intervening approval step
