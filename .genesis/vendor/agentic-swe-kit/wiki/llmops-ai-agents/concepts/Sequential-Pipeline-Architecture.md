...
## Graph RAG Graph Retrieval Augmented Generation traversing a knowledge graph of d
...
## Linear DAG Sequential Pipeline a directed acyclic graph where each stages output
...
## Quality Gate Pattern rejecting or halting low confidence items early in the pipe
...
## Short Circuit Logic skipping intermediate pipeline stages when a condition eg cr
...
## State Persistence per Stage storing each stages durable output so that on failur
