---
title: Common Sense Estimation: A pattern-matching heuristic mapping code structure to complexity class — simple loops → O(n), nested loops → O(n²), binary chop → O(log n), divide-and-conquer → O(n log n), combinatoric permutations → O(n!)
created: 2026-05-11
updated: 2026-05-11
type: concept
tags: [phase-1, pragmatic-programmer, concept]
sources: [extracts/pragmatic-programmer/32-Algorithm-Speed.json]
contributing_chapters: ["32. Algorithm Speed"]
confidence: high
---

# Common Sense Estimation: A pattern-matching heuristic mapping code structure to complexity class — simple loops → O(n), nested loops → O(n²), binary chop → O(log n), divide-and-conquer → O(n log n), combinatoric permutations → O(n!)

> From chapter: *32. Algorithm Speed*
